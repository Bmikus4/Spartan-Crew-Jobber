...
//
// A venue miss PROVISIONS A ROW. That is how the tenant came to hold 632 ExCeLs,
// so every branch here either books an existing venue or grows the tenant, and
// which one it does is worth pinning.
//
...
async function main() {
  ...
  console.log("\n[3] a CITY cannot identify a building");
  {
    ...
    ok(r.id === undefined, "no row is booked", String(r.id));
    ok(!!r.provision, "a new venue is created instead of guessing");
    ok(!!r.note && /names only a city/.test(r.note), "and the ticket says why", r.note ?? "(none)");
    const b = await go("Birmingham");
    ok(b.id === undefined, "Birmingham does not book the NEC", String(b.id));
  }
  ...
  console.log("\n[6] a venue the tenant genuinely does not hold is still provisioned");
  {
    const r = await go("Thornbury Assembly Rooms, 14 Kestrel Way, Thornbury BS35 2QQ");
    ok(r.id === undefined && !!r.provision, "a real new venue is created", JSON.stringify(r.provision?.name));
    ok(r.provision?.address === "Thornbury Assembly Rooms, 14 Kestrel Way, Thornbury BS35 2QQ",
       "with the client's address on it, unlike the placeholder");
  }
